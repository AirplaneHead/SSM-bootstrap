...
<script type="text/javascript">
        ...
        $('#btn_edit').on("click", function () {
            ...
            if(row.length==1){
                for (var i = 0; i < row.length; i++) {
                    ...
                    //打开弹出框：
                    $('#updatemyModal').modal();
                    $('#updatepname').val(row[i].name);
                    $('#updatepage').val(row[i].age);
                    $('#updateid').val(row[i].id);
    ...
    function mya(){
        ...
        //实例化一个对象：
        var person={"name":addpname,"age":addpage};
        //将对象转换成字符串
    ...
    function myb(){
        ...
        var addid=$('#updateid').val();

        //实例化一个对象：
        var person={"name":addpname,"age":addpage,"id":addid};
        //将对象转换成字符串
        var str=JSON.stringify(person);
        $.ajax({
            type:"POST" ,
    ...
    function submitQuery() {
        ...
        var age = $('#txt_search_statu').val();
        //实例化一个对象：
        var person={"name":addpname,"age":addpage,"pageSize":10,"pageNumber":1};
        //将对象转换成字符串
        var str=JSON.stringify(person);
        $.ajax({
            type:"post" ,
            url:"/getsecondall.action",
            data:"str="+str,
            success:function(msg){
                if(msg==1){
                    $('#cusTable').bootstrapTable('refresh');
            ...
            <form id="formSearch" class="form-horizontal">
                <div class="form-group" style="margin-top:15px">
                    ...
                    <div class="col-sm-4" style="text-align:left;">
                        <button type="button" onclick="submitQuery()"  id="btn_query" class="btn btn-primary">查询</button>
                        <!--"-->
                    </div>
                </div>
            </form>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="updateff_p" method="post">
                    ...
                    <input type="hidden" name="namepid" class="form-control" id="updateid" placeholder="id">
